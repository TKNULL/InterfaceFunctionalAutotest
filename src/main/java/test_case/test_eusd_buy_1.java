...
/**
 * EUSD购买流程一：用户购买EUSD下单后，匹配到承兑商生成订单；用户15分钟内无操作，系统自动取消订单，交易失败。
 * 前提：1、普通用户登录，
 ...
 *
 *      5、用户15分钟内无操作，系统自动取消订单
 *      6、取消订单后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为”已取消”
 *      7、取消订单后，承兑商【承兑】【承兑资产】资金解冻X
 *      8、取消订单后，承兑商【承兑】【收款订单】页面第一列显示该订单，订单状态为“已取消”
 *
 *
 * */

public class test_eusd_buy_1 {
    public static Log log = LogFactory.getLog(test_eusd_buy_1.class);
    @BeforeMethod
    public void before_methd() throws InterruptedException {
        log.info("当前线程是："+Thread.currentThread().getId());
        Thread.sleep(5*1000);
        log.info("test_eusd_buy_1,等待5秒");
    }
    ...
    public void before_class(ITestContext context) throws IOException {
        ...
        log.info("test_eusd_buy_1用例获得的手机账号是：");
    ...
    public void eusd_buy_1_1(ITestContext context) throws IOException {
        log.info("当前线程是："+Thread.currentThread().getId());
        log.info("eusd_buy_1_1，根据订单ID判断是否下单成功");
        //1、用户登录
        ...
        log.info("eusd_buy_1_1，查看下单返回订单的id是："+id);
        if (id!=null){
        ...
        log.info("eusd_buy_1_1，此处需要查看匹配的承兑商，是否是开启的那个承兑商");
        //预计匹配的承兑商
        String eu_user_mibile = (String) context.getAttribute("eu_user_mibile");
        //实际匹配的承兑商
        String get_eu_user_mobile= user.according_to_orders_id_return_eu_mobile();
        log.info("eusd_buy_1_1，预计匹配的承兑商是："+eu_user_mibile);
        log.info("eusd_buy_1_1，实际匹配的承兑商是："+get_eu_user_mobile);
        ...
        log.info("eusd_buy_1_1，测试结束");
    }




    @Test(dependsOnMethods = { "eusd_buy_1_1" },description = "下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”")
    //二、下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”
    public void eusd_buy_1_2(ITestContext context) throws IOException {
        log.info("当前线程是："+Thread.currentThread().getId());
        log.info("eusd_buy_1_2：下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”");
        String user_token = (String) context.getAttribute("user_token");
        User user = new User(user_token);
        int status = user.get_first_orders_status_by_id();        //查看普通用户，订单列表第一个订单，返回订单的状态，订单状态1为未付款
        log.info("eusd_buy_1_2:查看订单状态（1为正确）："+status);
        Assert.assertEquals(1,status);
        log.info("eusd_buy_1_2,测试结束");
    }



    @Test(dependsOnMethods ="eusd_buy_1_2",description = "下单成功后，承兑商【承兑】【承兑资产】资金冻结X")
    //三、下单成功后，承兑商【承兑】【承兑资产】资金冻结X
    public void eusd_buy_1_3(ITestContext context)throws IOException {
        log.info("当前线程是："+Thread.currentThread().getId());
        ...
        log.info("eusd_buy_1_3,用户下单后，承兑商冻结余额 + 下单数量 = 下单前余额,参与计算的数值是：");
        log.info("eusd_buy_1_3,eu_eusd_before_trade是："+eu_eusd_before_trade);
        log.info("eusd_buy_1_3,EUSD_buy_quantity是："+EUSD_buy_quantity);
        log.info("eusd_buy_1_3,eu_eusd_after_frozen是："+eu_eusd_after_frozen);
    ...
    @Test(dependsOnMethods = "eusd_buy_1_3"  ,description = "下单成功后，承兑商【承兑】【收款订单】页面第一列显示该订单，订单状态为“未付款”")
    //四、下单成功后，承兑商【承兑】【收款订单】页面第一列显示该订单，订单状态为“未付款”
    public void eusd_buy_1_4(ITestContext context) throws IOException {
        log.info("当前线程是："+Thread.currentThread().getId());
        log.info("eusd_buy_1_4，下单成功后，承兑商【承兑】【收款订单】页面第一列显示该订单，订单状态为“未付款”");
        String  eu_token = (String) context.getAttribute("eu_token");
        User eu_user = new User(eu_token);
        int status = eu_user.get_eu_orders_buy_status_by_id();
        log.info("eusd_buy_1_4,订单状态是(1为正确）："+status);
        Assert.assertEquals(1,status);
        log.info("eusd_buy_1_4，测试结束");
    }




    @Test(dependsOnMethods = { "eusd_buy_1_4" } ,description = "用户15分钟内无操作，系统自动取消订单" )
    //五、用户15分钟内无操作，系统自动取消订单
    public void eusd_buy_1_5(ITestContext context) throws IOException, InterruptedException {
        log.info("当前线程是："+Thread.currentThread().getId());
        log.info("eusd_buy_1_5,用户15分钟内无操作，系统自动取消订单");
        Thread.currentThread().sleep(15*60*1000) ;
        log.info("eusd_buy_1_5,测试结束");
    }


    @Test(dependsOnMethods = { "eusd_buy_1_5" } ,description = "用户取消后，查看【我的】【我的订单】页面第一个订单状态是否为已取消")
    //六、用户取消后，查看【我的】【我的订单】页面第一个订单状态是否为已取消
    public void eusd_buy_1_6(ITestContext context) throws IOException {
        log.info("当前线程是："+Thread.currentThread().getId());
        log.info("eusd_buy_1_6：订单取消后，查看普通用户的订单是否取消，订单状态是否为已过期（自动取消");
        String user_token = (String) context.getAttribute("user_token");
        User user = new User(user_token);
        int status = user.get_first_orders_status_by_id();
        log.info("eusd_buy_1_6：订单取消后，查看普通用户的订单是否取消，订单状态（6为正确:已过期（自动取消））是："+status);
        Assert.assertEquals(6,status);
        log.info("eusd_buy_1_6，测试结束");
    }


    @Test(dependsOnMethods = { "eusd_buy_1_6" } ,description = "用户取消后，查看承兑商，【承兑】【承兑资产】资金是否解冻")
    //七、用户取消后，查看承兑商，【承兑】【承兑资产】资金是否解冻；
    public void eusd_buy_1_7(ITestContext context) throws IOException {
        log.info("当前线程是："+Thread.currentThread().getId());
        log.info("eusd_buy_1_7,用户取消后，查看承兑商，【承兑】【承兑资产】资金是否解冻；");
        ...
        log.info("eusd_buy_1_7,冻结后的余额  +  用户下单金额（取消后被解冻金额）  = 现冻结余额，参与计算的数值是：");
        log.info("eusd_buy_1_7,eu_eusd_final_frozen是："+eu_eusd_final_frozen);
        log.info("eusd_buy_1_7,EUSD_buy_quantity："+EUSD_buy_quantity);
        log.info("eusd_buy_1_7,eu_eusd_after_frozen："+eu_eusd_after_frozen);
    ...
    @Test(dependsOnMethods = { "eusd_buy_1_7" } ,description = "用户取消后，查看承兑商，【承兑】【收款订单】页面，第一个订单的状态是否为已取消")
    //八、用户取消后，查看承兑商，【承兑】【收款订单】页面，第一个订单的状态是否为已取消；
    public void eusd_buy_1_8(ITestContext context) throws IOException {
        log.info("当前线程是："+Thread.currentThread().getId());
        log.info("eusd_buy_1_8，下单成功后，承兑商【承兑】【收款订单】页面第一列显示该订单，订单状态为“已过期自动取消”");
        String  eu_token = (String) context.getAttribute("eu_token");
        User eu_user = new User(eu_token);
        int status = eu_user.get_eu_orders_buy_status_by_id();
        log.info("eusd_buy_1_8,订单状态是(6为正确）："+status);
        Assert.assertEquals(6,status);
        log.info("eusd_buy_1_8，测试结束");
