...
/**
 * EUSD出售流程一：普通用户,出售下单后，匹配到承兑商生成订单；承兑商取消订单，交易失败。
 * 前提：1、普通用户登录，
...
public class test_eusd_sell_0 {
    public static Log log = LogFactory.getLog(test_eusd_sell_0.class);
    ...
    public void before_class(ITestContext context) throws IOException {
        ...
        log.info("test_eusd_sell_0用例获得的手机账号是：");
    ...
    public void eusd_sell_0_1(ITestContext context) throws IOException {
        log.info("eusd_sell_0_1,用户下单，查看是否下单成功");
        //1、用户下单，判断是否生成订单
    ...
    @Test(dependsOnMethods = { "eusd_sell_0_1" },description ="下单成功后，用户[资产][EUSD]可用余额-x,冻结金额+x" )
    //二、下单成功后，用户[资产][EUSD]可用余额-x,冻结金额+x
    public void eusd_sell_0_2(ITestContext context) throws IOException, InterruptedException {
        ...
        log.info("eusd_sell_0_2,用户原可用余额-出售金额=现可用余额,则测试通过,参与计算的数值是:");
        log.info("user_eusd_before_available:" + user_eusd_before_available);
        ...
        log.info("eusd_sell_0_2,用户原冻结余额+出售金额=现冻结余额,则测试通过,参与计算的数值是:");
        ...
        log.info("eusd_sell_0_2,测试结束");
    }



    @Test(dependsOnMethods = { "eusd_sell_0_2" } ,description ="下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”")
    //三、下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”
    public void eusd_sell_0_3(ITestContext context) throws IOException {
        //1.用户登录
        log.info("eusd_sell_0_3：下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”");
        String user_token = (String) context.getAttribute("user_token");
        User user = new User(user_token);
        //2.查看订单状态
        int status = user.get_first_orders_status_by_id();
        log.info("eusd_sell_0_3:查看订单状态(1为正确)："+status);
        Assert.assertEquals(1,status);
        log.info("eusd_sell_0_3,测试结束");
    ...
    @Test(dependsOnMethods = { "eusd_sell_0_3" } ,description ="下单成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“未付款”")
    //四、下单成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“未付款”
    public void eusd_sell_0_4(ITestContext context) throws IOException {
        log.info("eusd_sell_0_4:下单成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“未付款”");
        //1.承兑商登录
        ...
        log.info("eusd_sell_0_4,订单状态是(1为正确)："+status);
        Assert.assertEquals(1,status);
        log.info("eusd_sell_0_4，测试结束");
    }




    @Test(dependsOnMethods = { "eusd_sell_0_4" },description ="承兑商点击【取消订单】，查看取消是否成功")
    //五、承兑商点击【取消订单】，查看取消是否成功
    public void eusd_sell_0_5(ITestContext context) throws IOException, InterruptedException {
        log.info("eusd_sell_0_5:承兑商点击【取消订单】，查看取消是否成功");
        BigInteger id_by_cancel = (BigInteger) context.getAttribute("id");
        String eu_token = (String) context.getAttribute("eu_token");
        //1.承兑商登录
        User eu_user = new User(eu_token);
        //2.传入ID取消订单
        int result_code =eu_user.sell_cancel(id_by_cancel);
        log.info("eusd_sell_0_5,服务器返回的状态码是(200为正确)："+result_code);
        Assert.assertEquals(200,result_code);
        log.info("eusd_sell_0_5,测试结束");
    }




    @Test(dependsOnMethods = { "eusd_sell_0_5" },description ="取消订单后，用户【资产】【EUSD】模块资产可用余额+X，冻结余额-X")
    //六、取消订单后，用户【资产】【EUSD】模块资产可用余额+X，冻结余额-X
    public void eusd_sell_0_6(ITestContext context) throws IOException, InterruptedException {
        log.info("eusd_sell_0_6:取消订单后，用户【资产】【EUSD】模块资产可用余额+X，冻结余额-X");
        Thread.sleep(3*60*1000);
        boolean result_1;
        boolean result_2;
        //1.用户登录
        ...
        log.info("eusd_sell_0_6,用户原可用余额=现可用余额,则测试通过,参与计算的数值是:");
        log.info("user_eusd_before_available:"+user_eusd_before_available);
        log.info("user_after_available:"+user_after_available);
        ...
        log.info("result_1是:"+result_1);
        log.info("eusd_sell_0_6,用户原冻结余额=现冻结余额,则测试通过,参与计算的数值是:");
        log.info("user_after_frozen:"+user_after_frozen);
        log.info("user_eusd_before_frozen:"+user_eusd_before_frozen);
        ...
        log.info("eusd_sell_0_6,测试结束");
    }




    @Test(dependsOnMethods = { "eusd_sell_0_6" },description ="取消订单后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“已取消”")
    //七、取消订单后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“已取消”
    public void eusd_sell_0_7(ITestContext context) throws IOException {
        log.info("eusd_sell_0_7:取消订单后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“已取消”");
        String user_token = (String) context.getAttribute("user_token");
        User user = new User(user_token);
        int status = user.get_first_orders_status_by_id();
        log.info("eusd_sell_0_7:查看订单状态(4为正确)："+status);
        Assert.assertEquals(4,status);
        log.info("eusd_sell_0_7,测试结束");
    }



    @Test(dependsOnMethods = { "eusd_sell_0_7" },description ="取消订单后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“已取消”")
    //八、取消订单后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“已取消”
    public void eusd_sell_0_8(ITestContext context) throws IOException {
        log.info("eusd_sell_0_8:取消订单后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“已取消”");
        String eu_token = (String) context.getAttribute("eu_token");
        User eu_user = new User(eu_token);
        int status = eu_user.get_eu_orders_buy_status_by_id_side_2();
        log.info("eusd_sell_0_8,订单状态是(4为正确)："+status);
        Assert.assertEquals(4,status);
        log.info("eusd_sell_0_8，测试结束");
