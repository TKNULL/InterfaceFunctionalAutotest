...
/**
 * EUSD出售流程三：普通用户,出售下单后，匹配到承兑商生成订单；承兑商确认付款；用户确认订单，交易成功。
 * 前提：1、普通用户登录，
 ...
 *
 *      7、用户点击【我已收款】确认收款，查看是否成功（订单详情页面，点击【我已收款】）                                                                     ===》7、承兑商点击【取消订单】，查看是否取消成功  &&   7、用户5分钟内无操作，订单逾期，此时用户无法点击【我已收款】，会提示”订单状态异常“
...
public class test_eusd_sell_2 {
    public static Log log = LogFactory.getLog(test_eusd_sell_2.class);
    @BeforeMethod
    ...
    public void before_class(ITestContext context) throws IOException {
        ...
        log.info("test_eusd_sell_2用例获得的手机账号是：");
    ...
    public void eusd_sell_2_1(ITestContext context) throws IOException {
        log.info("eusd_sell_2_1,用户下单，查看是否下单成功");
        //1、用户下单，判断是否生成订单
    ...
    @Test(dependsOnMethods = { "eusd_sell_2_1" }   , description ="下单成功后，用户[资产][EUSD]可用余额-x,冻结金额+x"    )
    //二、下单成功后，用户[资产][EUSD]可用余额-x,冻结金额+x
    public void eusd_sell_2_2(ITestContext context) throws IOException, InterruptedException {
        log.info("eusd_sell_2_2:下单成功后，用户[资产][EUSD]可用余额-x,冻结金额+x");
        ...
        log.info("eusd_sell_2_2,用户原可用余额-出售金额=现可用余额,则测试通过,参与计算的数值是:");
    ...
    @Test(dependsOnMethods = { "eusd_sell_2_2" }, description ="下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”")
    //三、下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”
    public void eusd_sell_2_3(ITestContext context) throws IOException {
        //1.用户登录
        log.info("eusd_sell_2_3：下单成功后，用户【我的】【我的订单】页面第一列显示该订单，且订单状态为“未付款”");
        String user_token = (String) context.getAttribute("user_token");
        User user = new User(user_token);
        //2.查看订单状态
        int status = user.get_first_orders_status_by_id();
        log.info("eusd_sell_2_3:查看订单状态(1为正确)："+status);
        Assert.assertEquals(1,status);
        log.info("eusd_sell_2_3,测试结束");

    }


    @Test(dependsOnMethods = { "eusd_sell_2_3" }, description ="下单成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“未付款”")
    //四、下单成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“未付款”
    public void eusd_sell_2_4(ITestContext context) throws IOException {
        log.info("eusd_sell_2_4:下单成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“未付款”");
        //1.承兑商登录
        ...
        log.info("eusd_sell_2_4,订单状态是(1为正确)："+status);
        Assert.assertEquals(1,status);
        log.info("eusd_sell_2_4，测试结束");
    }
    ...
    @Test(dependsOnMethods = { "eusd_sell_2_4" }, description ="承兑商点击【我已付款】，查看是否付款成功")
    //五、承兑商点击【我已付款】，查看是否付款成功
    public void eusd_sell_2_5(ITestContext context) throws IOException {
        log.info("eusd_sell_2_5,承兑商点击【我已付款】，查看是否付款成功");
        String eu_token = (String) context.getAttribute("eu_token");
        BigInteger id = (BigInteger) context.getAttribute("id");
        User  eu_user = new User(eu_token);
        int code=  eu_user.sell_id_pay(id);
        log.info("eusd_sell_2_5,返回数据（200表示成功）是："+code);
        Assert.assertEquals(200,code);
        log.info("eusd_sell_2_5,测试完成");
    }



    @Test(dependsOnMethods = { "eusd_sell_2_5" }, description ="承兑商付款成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“已付款”")
    //六、承兑商付款成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“已付款”
    public void eusd_sell_2_6(ITestContext context) throws IOException {
        log.info("eusd_sell_2_6:下单成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为“已付款”");
        String eu_token = (String) context.getAttribute("eu_token");
        User eu_user = new User(eu_token);
        //获取订单状态
        int status = eu_user.get_eu_orders_buy_status_by_id_side_2();
        log.info("eusd_sell_2_6,订单状态是(2为正确)："+status);
        Assert.assertEquals(2,status);
        log.info("eusd_sell_2_6，测试结束");
    }



    @Test(dependsOnMethods = { "eusd_sell_2_6" }, description ="承兑商付款成功后，用户【我的】【我的订单】页面第一列显示该订单，订单状态为“已付款”")
    //七、承兑商付款成功后，用户【我的】【我的订单】页面第一列显示该订单，订单状态为"已付款"
    public void eusd_sell_2_7(ITestContext context) throws IOException {
        log.info("eusd_sell_2_7:承兑商付款成功后，用户【我的】【我的订单】页面第一列显示该订单，订单状态为”已付款“");
        String user_token = (String) context.getAttribute("user_token");
        User eu_user = new User(user_token);
        int status = eu_user.get_first_orders_status_by_id();
        log.info("eusd_sell_2_7,点击确认支付后，查看订单状态(2为正确)："+status);
        Assert.assertEquals(2,status);
        log.info("eusd_sell_2_7，测试结束");
    }


    @Test(dependsOnMethods = { "eusd_sell_2_7" }, description ="用户点击【我已收款】确认收款，查看是否成功")
    //八、用户点击【我已收款】确认收款，查看是否成功
    public void eusd_sell_2_8(ITestContext context) throws IOException {
        log.info("eusd_sell_2_8:用户点击【我已收款】确认收款，查看是否成功");
        String user_token = (String) context.getAttribute("user_token");
        BigInteger id = (BigInteger) context.getAttribute("id");
        User user = new User(user_token);
        int code = user.v1_sell_id_confirm(id);
        log.info("eusd_sell_2_8,返回的状态码（200表示正确）是："+code);
        Assert.assertEquals(200,code);
        log.info("eusd_sell_2_8,测试结束");
    }



    @Test(dependsOnMethods = { "eusd_sell_2_8" }, description ="用户收款成功后，用户【资产】【EUSD】可用余额-X")
    //九、用户收款成功后，用户【资产】【EUSD】可用余额-X
    //用户收款成功后，用户【资产】【EUSD】冻结余额-X（用户下单的时候，可用余额先转到冻结余额，承兑商付款的时候，冻结余额减少，转到承兑商账户）
    public void eusd_sell_2_9(ITestContext context) throws IOException {
        log.info("eusd_sell_2_9:用户收款成功后，用户【资产】【EUSD】冻结余额-X（用户下单的时候，可用余额先转到冻结余额，承兑商付款的时候，冻结余额减少，转到承兑商账户）");
        String user_token = (String) context.getAttribute("user_token");
        User user = new User(user_token);
        ...
        log.info("eusd_sell_2_9,//前冻结余额 - 下单出售金额 = 现冻结余额,则测试通过 参与计算的金额是：");
        log.info("user_after_frozen="+user_after_frozen);
        log.info("EUSD_sell_quantity="+EUSD_sell_quantity);
        ...
        log.info("eusd_sell_2_9,测试结束");
    }


    @Test(dependsOnMethods = { "eusd_sell_2_9" }, description ="用户收款成功后，【我的】【我的订单】页面第一列显示该订单，订单状态为”已确认“")
    //十、用户收款成功后，【我的】【我的订单】页面第一列显示该订单，订单状态为"已确认"
    public void eusd_sell_2_10(ITestContext context) throws IOException {
        log.info("eusd_sell_2_10：用户收款成功后，【我的】【我的订单】页面第一列显示该订单，订单状态为“已确认”");
        String user_token= (String) context.getAttribute("user_token");
        User  user = new User(user_token);
        int status = user.get_first_orders_status_by_id();
        log.info("eusd_sell_2_10:查看订单状态(3为正确)："+status);
        Assert.assertEquals(3,status);
        log.info("eusd_sell_2_10,测试结束");
    }




    @Test(dependsOnMethods = { "eusd_sell_2_10" }, description ="用户收款成功后，承兑商【承兑】【承兑资产】EUSD数值可用余额不变，冻结余额+X")
    //十、用户收款成功后，承兑商【承兑】【承兑资产】EUSD数值可用余额不变，冻结余额+X            冻结余额是否立即+x
    public void eusd_sell_2_11(ITestContext context) throws IOException, InterruptedException {
        log.info("eusd_sell_2_11:用户收款成功后，承兑商【承兑】【承兑资产】EUSD数值可用余额不变，冻结余额+X，");
        ...
        BigDecimal EUSD_sell_quantity = (BigDecimal) context.getAttribute("EUSD_sell_quantity");
        log.info("eusd_sell_2_11,原有冻结金额 + 下单金额 = 现有冻结金额,测试通过,参与计算的数值是：");
        log.info("eusd_sell_2_11，eu_eusd_before_trade=："+eu_eusd_before_trade);   // null
        log.info("eusd_sell_2_11，EUSD_sell_quantity=："+EUSD_sell_quantity);
        log.info("eusd_sell_2_11，final_euuser_after_trade=："+final_euuser_after_trade);
        ...
        log.info("eusd_sell_2_11,测试结束");
    }


    @Test(dependsOnMethods = { "eusd_sell_2_11" },description ="用户收款成功后，承兑商【承兑】【承兑资产】EUSD数值可用余额不变，冻结余额+X：4分钟后，冻结余额—X，可用余额+x")
    //十一、用户收款成功后，承兑商【承兑】【承兑资产】EUSD数值可用余额不变，冻结余额+X：4分钟后，冻结余额—X，可用余额+x
    public void eusd_sell_2_12(ITestContext context) throws IOException, InterruptedException {
        log.info("eusd_sell_2_12:用户收款成功后，承兑商【承兑】【承兑资产】EUSD数值可用余额不变，冻结余额+X：4分钟后，冻结余额—X，可用余额+x");
        boolean result1;
        boolean result2;
        ...
        log.info("eusd_sell_2_12,承兑商可用余额增加，参数计算的数值是：");
        log.info("eusd_sell_2_12,eu_eusd_before_available："+eu_eusd_before_available);        // null
        log.info("eusd_sell_2_12,EUSD_sell_quantity是："+EUSD_sell_quantity);
        log.info("eusd_sell_2_12,euuser_after_availabl是："+euuser_after_availabl);
        ...
        log.info("eusd_sell_2_12,承兑商冻结余额减少，参数计算的数值是：");
        log.info("eusd_sell_2_12,euuser_after_trade是："+euuser_after_trade);
        log.info("eusd_sell_2_12,EUSD_sell_quantity是："+EUSD_sell_quantity);
        log.info("eusd_sell_2_12,final_euuser_after_trade是："+final_euuser_after_trade);
        ...
        log.info("eusd_sell_2_12，测试结束");
    }



    @Test(dependsOnMethods = { "eusd_sell_2_12" },description ="用户收款成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为”已确认“")
    //十二、用户收款成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为”已确认“
    public void eusd_sell_2_13(ITestContext context) throws IOException {
        log.info("eusd_sell_2_13:下单成功后，用户收款成功后，承兑商【承兑】【支付订单】页面第一列显示该订单，订单状态为”已确认“");
        String eu_token = (String) context.getAttribute("eu_token");
        User eu_user = new User(eu_token);
        //获取订单状态
        int status = eu_user.get_eu_orders_buy_status_by_id_side_2();
        log.info("eusd_sell_2_13,订单状态是(3为正确)："+status);
        Assert.assertEquals(3,status);
        log.info("eusd_sell_2_13，测试结束");

    }

    @Test(dependsOnMethods = { "eusd_sell_2_13" },description ="用户收款成功后，承兑商【承兑】【支付】今日累积支付金额+X")
    //十三、用户收款成功后，承兑商【承兑】【支付】今日累积支付金额+X
    public void eusd_sell_2_14(ITestContext context) throws IOException {
        log.info("eusd_sell_2_14:用户收款成功后，承兑商【承兑】【支付】今日累积支付金额+X");
        String eu_token = (String) context.getAttribute("eu_token");
        User eu_user = new User(eu_token);
        ...
        log.info("eusd_sell_2_14,参与比较的两个数字（after_sell_rmb_today>sell_rmb_today说明测试通过）是：");
        log.info("eusd_sell_2_14,after_sell_rmb_today是："+after_sell_rmb_today);
        log.info("eusd_sell_2_14,eu_eusd_before_buy_rmb_today："+eu_eusd_before_buy_rmb_today);
        if((after_sell_rmb_today.compareTo(eu_eusd_before_buy_rmb_today))>0){
            Assert.assertTrue(true);
        }else{
            Assert.assertFalse(false);
        }
        log.info("eusd_sell_2_14,测试结束");
    }
